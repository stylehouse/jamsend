...
async function hello(cli, soul, from, want) {
  const ts = Date.now();
...
async function main() {
  ...
  attachRelay(srv);
  const soul = await mint();
  const P = prepubOf(soul.pubHex);
  const bodyKey = await mint();
  ...
  const bodyA = browser(port, "");
  await bodyA.open;
  await hello(bodyA, soul, P, foreign);
  ...
  const ghost = browser(port, P);
  await ghost.open;
  await hello(ghost, soul, P, P);
  await until(() => ghost.ctrl.some((m) => m.control === "hello_ok"));
  const live = browser(port, "");
  await live.open;
  await hello(live, soul, P, P);
  ...
  const sender = browser(port, "somefriend");
  await sender.open;
  ...
  const ridA = "7011", ridB = "7022";
  const bA = browser(port, P2 + "_" + ridA);
  await bA.open;
  await hello(bA, soul2, P2 + "_" + ridA, P2 + "_" + ridA);
  const bB = browser(port, P2 + "_" + ridB);
  await bB.open;
