        ...
        <div class="peer" title="prepub: {s.prepub}">
            ...
            <div class="row row1"
                ...
                <div class="c hs" title="Hello handshake: exchange of full publicKeys">
                    <span class="arrow" class:y={s.said_hello} class:n={!s.said_hello}
                    ...
                    <span class="arrow" class:y={s.heard_hello} class:n={!s.heard_hello}
                        title="↓ heard_hello: got their publicKey (Ud)">↓</span>
                </div>

                <div class="c hs" title="Trust handshake: exchange of signed trust certs">
                    <span class="arrow" class:y={s.said_trust} class:n={!s.said_trust}
                    ...
                    <span class="arrow" class:y={s.heard_trust} class:n={!s.heard_trust}
            ...
            <!-- ROW 2: age | garden | idzeug(flex) | ping | grants -->
            <div class="row row2"
                class:ready={s.lp_ready}
                class:disc={!s.con_open && s.has_instance}
                class:ghost={!s.has_instance}>

                <div class="c age" title="introduced: {s.introduced_at ? ago(s.introduced_at)+' ago' : 'never'}">
                    <span>{#if s.introduced_at}{ago(s.introduced_at)}{/if}</span>

                    <!-- these two are pushed to the right end by 1fr on .iz -->
                    <span class="ping" title="Ping health">
                        {#if s.ping_good}
                ...
                <div class="c ga" title="Gardening state">
...
.row1 {
    /* id      con      ↑hi↓   ↑trust↓   $trust(ed) */
    grid-template-columns: 80px 80px auto auto auto;
    border-radius: 2px 2px 0 0;
}
.row2 {
    /* age    garden   idzeug  ping   grants */
...
.age { font-size: 8px; color: #2a4a2a; }
.ga  { font-size: 9px; }
