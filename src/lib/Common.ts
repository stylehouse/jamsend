// Common — the plain-JS floor under everything: hashing + identity crypto, the clock, text peel|depeel,
//  the C-shaped helpers (ex/tex/hak…), erring, throttle.  Traditional imports, not eatfunc'd onto H|A.
//   Was Y.svelte.ts (+ O/Hashly.ts, merged 2026-09-16 — the byte hasher belongs beside the string one).
import { TheC } from "./Stuff.svelte";
import * as ed from '@noble/ed25519';
import { sha256 as sha256_bytes } from '@noble/hashes/sha2.js';
...
//#region crypto
//#region bytes hashing (né O/Hashly.ts)
// the byte-identity hasher: sha256 as lowercase hex, one-shot AND incremental.
...
export function sha256_hex(bytes: Uint8Array): string {
    return bytesToHex(sha256_bytes(bytes));
}
