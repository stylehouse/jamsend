...
VytoGuise(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.VytoGuise_drive(w,req)
        req.sc.ok = 1
...
VytoGuise_sow(w) {
    w.i({desc: 'build a result set — six flat hits and a collapsed tail from plain objects not particles'})
    w.c.seeds = []
...
async VytoGuise_stand(w) {
    w.i({desc: 'stand the flat set — every hit is its own cell, and the shared-scalar ties are drawn as vines landing on the word'})
...
VytoGuise_witness(w) {
    if (w.c.saw_stand) {
        ...
        if (!(w.oa({see: 'a flat result set stands as cells — no grouping cell anywhere, only the hits themselves'}))) w.i({see: 'a flat result set stands as cells — no grouping cell anywhere, only the hits themselves'})
    }
    if (w.c.saw_voice) {
    ...
    if (w.c.saw_sealed) {
        ...
        if (!(w.oa({see: 'a guise says only what it names — the producer private field sits beside the spec and never reaches a cell'}))) w.i({see: 'a guise says only what it names — the producer private field sits beside the spec and never reaches a cell'})
    }
    if (w.c.saw_drawn) {
        ...
        if (!(w.oa({see: 'the grouping is drawn and not contained — no cell holds the hits yet every tie names the shared scalar it is about'}))) w.i({see: 'the grouping is drawn and not contained — no cell holds the hits yet every tie names the shared scalar it is about'})
    }
    if (w.c.saw_morph) {
