...
import { poly_area } from "$lib/O/vyto_geometry"
    ...
    Ghostmeta_Ghost_V_VytoTesting(): string { return '92d479f4f985e86b~g1' },
...
VytoMembrane(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.VytoMembrane_drive(w,req)
        req.sc.ok = 1
...
VytoMembrane_seed(w) {
    w.i({desc: 'seed the orchestra world — a band of three players beside three songs of one artist and two strays'})
    let band = w.i({ Band: 'main' })
    band.i({ Player: 'keys', of: 'main' })
    band.i({ Player: 'bass', of: 'main' })
    band.i({ Player: 'drums', of: 'main' })
...
async VytoMembrane_stand(w) {
    w.i({desc: 'commission foam-cut and nested with membrane on the deck — the songs and the players each become one family scope'})
...
async VytoMembrane_diverge(w) {
    w.i({desc: 'change one song artist — the shared artist must leave the bump while the family stands'})
...
async VytoMembrane_shrink(w) {
    w.i({desc: 'drop one song — the family must count two and still stand'})
...
async VytoMembrane_dissolve(w) {
    w.i({desc: 'drop a second song — one song is no family so the membrane dissolves and the survivor moves back to the top'})
...
VytoMembrane_witness(w) {
    if (w.c.saw_stand) {
        ...
        if (!(w.oa({see: 'two families stand as two scopes — the songs share their artist on the bump and the players their of'}))) w.i({see: 'two families stand as two scopes — the songs share their artist on the bump and the players their of'})
    }
    if (w.c.saw_diverge) {
        this.story_swear(w, 'a diverging artist leaves the bump — what is shared is only what every member says')
        if (!(w.oa({see: 'a diverging artist leaves the bump — what is shared is only what every member says'}))) w.i({see: 'a diverging artist leaves the bump — what is shared is only what every member says'})
    }
    if (w.c.saw_shrink) {
        this.story_swear(w, 'a departed song shrinks the family to two — the membrane counts what it holds')
        if (!(w.oa({see: 'a departed song shrinks the family to two — the membrane counts what it holds'}))) w.i({see: 'a departed song shrinks the family to two — the membrane counts what it holds'})
    }
    if (w.c.saw_dissolve) {
        this.story_swear(w, 'one song is no family — the membrane dissolves and the survivor moves back out to the top')
        if (!(w.oa({see: 'one song is no family — the membrane dissolves and the survivor moves back out to the top'}))) w.i({see: 'one song is no family — the membrane dissolves and the survivor moves back out to the top'})
    }
...
VytoSeem(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.VytoSeem_drive(w,req)
        req.sc.ok = 1
...
VytoSeem_sow(w) {
    w.i({desc: 'sow a clustered result set — two facet clusters of three hits and a collapsed tail from plain trees not particles'})
    w.c.seeds = []
...
async VytoSeem_stand(w) {
    w.i({desc: 'stand the sown set foam-cut and nested — the clusters tile and the tail speaks its count'})
...
async VytoSeem_resow(w) {
    w.i({desc: 're-sow the same hits with new scores — the producer id is the identity so each cell morphs in place'})
    let vw = this.VytoStaple_vw(w)
    if (!vw) return
    w.c.was_row = {}
    w.c.was_score = {}
...
VytoSeem_witness(w) {
    if (w.c.saw_stand) {
        ...
        if (!(w.oa({see: 'a sown result set stands as cells — plain trees laid out by the same cut and folio with no particle of the program grappled'}))) w.i({see: 'a sown result set stands as cells — plain trees laid out by the same cut and folio with no particle of the program grappled'})
    }
    if (w.c.saw_voice) {
        this.story_swear(w, 'a collapsed cluster speaks its count and its facets — a voice with no fold behind it')
        if (!(w.oa({see: 'a collapsed cluster speaks its count and its facets — a voice with no fold behind it'}))) w.i({see: 'a collapsed cluster speaks its count and its facets — a voice with no fold behind it'})
    }
    if (w.c.saw_sealed) {
        this.story_swear(w, 'the facts are an allowlist — the producer private field sits beside the spec and never reaches a cell')
        if (!(w.oa({see: 'the facts are an allowlist — the producer private field sits beside the spec and never reaches a cell'}))) w.i({see: 'the facts are an allowlist — the producer private field sits beside the spec and never reaches a cell'})
    }
    if (w.c.saw_inflate) {
        ...
        if (!(w.oa({see: 'a non-scalar fact inflates instead of vanishing — a nested record grows its own cell and a list becomes a spread of chips'}))) w.i({see: 'a non-scalar fact inflates instead of vanishing — a nested record grows its own cell and a list becomes a spread of chips'})
    }
    if (w.c.saw_morph) {
