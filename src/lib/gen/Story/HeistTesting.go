...
import { mint_grant } from "$lib/O/Funk/Grant.ts"
    ...
    Ghostmeta_Ghost_Story_HeistTesting(): string { return '122744d5f3993452~g1' },
...
MusuLossy(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuLossy_drive(w,req)
        req.sc.ok = 1
...
MusuPoolPolicy(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuPoolPolicy_drive(w,req)
        req.sc.ok = 1
...
MusuPoolPolicy_sediment(w) { const H = this;
...
MusuPoolPolicy_barred(w) { const H = this;
...
MusuPoolRandom(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuPoolRandom_drive(w,req)
        req.sc.ok = 1
...
MusuPoolRadio(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuPoolRadio_drive(w,req)
        req.sc.ok = 1
...
MusuPoolBytes(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuPoolBytes_drive(w,req)
        req.sc.ok = 1
...
MusuHeard(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuHeard_drive(w,req)
        req.sc.ok = 1
