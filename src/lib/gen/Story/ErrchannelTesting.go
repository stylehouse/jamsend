<script lang="ts">
    ...
    Ghostmeta_Ghost_Story_ErrchannelTesting(): string { return '55f4a7dd9c9193f1~g1' },
...
ErrChannel(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.ErrChannel_drive(w,req)
        req.sc.ok = 1
...
async ErrChannel_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
SuperCull(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SuperCull_drive(w,req)
        req.sc.ok = 1
...
SuperCull_stand(w) { const H = this;
...
async SuperCull_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
