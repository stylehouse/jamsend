...
MusuEdge(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuEdge_drive(w,req)
        req.sc.ok = 1
...
MusuEdge_witness(w) {
    ...
    //    always-true).
    if (c_over === 0 && c_min >= 0.3 && !(w.oa({witnessed: "holds_margin"}))) w.i({witnessed: "holds_margin"})
    ...
    if (c_throttles >= 8 && c_minrate <= 0.85 && !(w.oa({witnessed: "backs_off"}))) w.i({witnessed: "backs_off"})
    ...
    if (c_final >= 0.2 && c_final <= 1.0 && !(w.oa({witnessed: "low_latency"}))) w.i({witnessed: "low_latency"})
    ...
    if (b_over > 0 && !(w.oa({witnessed: "baseline_overruns"}))) w.i({witnessed: "baseline_overruns"})
    ...
    if (b_gaps - c_gaps >= 3 && b_gaps > 0 && c_bits >= 4 && !(w.oa({witnessed: "fewer_gaps"}))) w.i({witnessed: "fewer_gaps"})

},
async MusuEdge_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
MusuPier(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuPier_drive(w,req)
        req.sc.ok = 1
...
MusuPier_witness(w) {
    ...
    if (this.Lake_port(w.c.tx) && this.Lake_port(rx) && this.Lake_port(ltx) && !(w.oa({witnessed: "linked"}))) w.i({witnessed: "linked"})
    ...
    if (got.length >= 5 && got[0] === 1 && in_order(got) && !(w.oa({witnessed: "crossed"}))) w.i({witnessed: "crossed"})
    ...
    if (corrupt != null && got.indexOf(corrupt) < 0 && got.length >= 5 && !(w.oa({witnessed: "verified"}))) w.i({witnessed: "verified"})
    ...
    if (dropped > 0 && lgot.length >= 4 && in_order(lgot) && !(w.oa({witnessed: "dropped_then_healed"}))) w.i({witnessed: "dropped_then_healed"})

},
async MusuPier_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
MusuReplica_witness(w) {
    ...
    if (s0 && rec0 && +(s0.sc.have || 0) === +(rec0.sc.nchunks || 0) && +(rec0.sc.nchunks || 0) > 0 && s0.c.pages && s0.c.pages.length >= 3 && !(w.oa({witnessed: "complete"}))) w.i({witnessed: "complete"})
...
MusuReco_witness(w) {
    ...
    if (reco && sreco && reco.sc.note === sreco.sc.note && reco.sc.by === 'DJ' && !(w.oa({witnessed: "recommended"}))) w.i({witnessed: "recommended"})
    // refused_unstarted: the gate held — the un-started track was refused AND nothing of it crossed.
    if (w.oa({ refused: 'not_started' }) && !lib.oa({ Record: 1, id: 'trk1' }) && !(w.oa({witnessed: "refused_unstarted"}))) w.i({witnessed: "refused_unstarted"})
    // started_early: audio reached B while A's transcode still ran (captured at its moment, beat 4).
    if (w.oa({ early: 'before_transcode_done' }) && !(w.oa({witnessed: "started_early"}))) w.i({witnessed: "started_early"})
    ...
    if (s && rec && +(s.sc.have || 0) === +(rec.sc.nchunks || 0) && +(rec.sc.nchunks || 0) > 0 && want_samples > 0 && got_samples === want_samples && !(w.oa({witnessed: "complete"}))) w.i({witnessed: "complete"})
    // real_music: the tracks are decoded FILES off the nav walk — real flag, real duration, real artist.
    if (src0 && src0.sc.real && +(src0.sc.seconds || 0) >= 2 && (src0.sc.artist || '') !== '' && !(w.oa({witnessed: "real_music"}))) w.i({witnessed: "real_music"})
...
MusuConceal(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuConceal_drive(w,req)
        req.sc.ok = 1
...
MusuConceal_witness(w) {
    ...
    if (raw_s >= 4 && !(w.oa({witnessed: "has_gaps"}))) w.i({witnessed: "has_gaps"})
    // repeat_fills: repeating the last frame removed the silence (fewer silent windows than raw, down to 0).
    if (rep_s < raw_s && rep_s === 0 && !(w.oa({witnessed: "repeat_fills"}))) w.i({witnessed: "repeat_fills"})
    // pingpong_fills: reverse-pingpong also removed the silence.
    if (png_s === 0 && raw_s > 0 && !(w.oa({witnessed: "pingpong_fills"}))) w.i({witnessed: "pingpong_fills"})
    // real_fill: the concealment is REAL audio (entropy ~7), not silence relabelled.
    if (rep_b >= 4 && png_b >= 4 && !(w.oa({witnessed: "real_fill"}))) w.i({witnessed: "real_fill"})
    ...
    if (png_seam < rep_seam && rep_seam > 0.01 && !(w.oa({witnessed: "smoother"}))) w.i({witnessed: "smoother"})

},
async MusuConceal_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
RepliUpsert(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.RepliUpsert_drive(w,req)
        req.sc.ok = 1
...
RepliSplit(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.RepliSplit_drive(w,req)
        req.sc.ok = 1
...
RepliShadow(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.RepliShadow_drive(w,req)
        req.sc.ok = 1
...
ParkCull(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.ParkCull_drive(w,req)
        req.sc.ok = 1
...
MusuRadioAim(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.MusuRadioAim_drive(w,req)
        req.sc.ok = 1
