<script lang="ts">
    ...
    Ghostmeta_Ghost_Story_Sounditron(): string { return 'ef555d2e148876bb~g1' },
...
Sounditron_witness(w) {
    ...
    if (w.o({ Audio: 1 })[0]?.sc?.real && !(w.oa({seen: 'the sound system answered — a real AudioContext ran here'}))) w.i({seen: 'the sound system answered — a real AudioContext ran here'})
    if (n != null && n === 6 && w.o({ Audio: 1 })[0] && !w.o({ Audio: 1 })[0].sc.real && !(w.oa({log: 'no live audio — the context never ticked in real time'}))) w.i({log: 'no live audio — the context never ticked in real time'})
