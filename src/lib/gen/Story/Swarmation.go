...
import { sas_transcript, sas_row, sas_agree } from "$lib/O/Funk/Emojiconfirm.ts"
    ...
    Ghostmeta_Ghost_Story_Swarmation(): string { return '34ec9947a4543b33~g1' },
...
SwarmShare(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmShare_drive(w,req)
        req.sc.ok = 1
...
async SwarmShare_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
SwarmChain(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmChain_drive(w,req)
        req.sc.ok = 1
...
async SwarmChain_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
SwarmCohort(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmCohort_drive(w,req)
        req.sc.ok = 1
...
async SwarmCohort_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
SwarmRole(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmRole_drive(w,req)
        req.sc.ok = 1
...
async SwarmRole_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
SwarmSeal(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmSeal_drive(w,req)
        req.sc.ok = 1
...
async SwarmSeal_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
SwarmFerry(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmFerry_drive(w,req)
        req.sc.ok = 1
...
async SwarmFerry_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
EmojiConfirm(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.EmojiConfirm_drive(w,req)
        req.sc.ok = 1
...
async EmojiConfirm_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) return
...
SwarmSpread(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmSpread_drive(w,req)
        req.sc.ok = 1
...
async SwarmSpread_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) { return }
...
SwarmReboot(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmReboot_drive(w,req)
        req.sc.ok = 1
...
async SwarmReboot_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) { return }
...
SwarmHelm(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.SwarmHelm_drive(w,req)
        req.sc.ok = 1
...
async SwarmHelm_order(w) { const H = this;
    let As = H.o({A: 1})
    if (!As.length) { return }
