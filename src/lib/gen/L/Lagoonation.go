...
LagoonStaple(A,w) {
    w.doai({req: "wrangle", eternal: 1})?.(async (req) => {
        await this.LagoonStaple_drive(w,req)
        req.sc.ok = 1
...
LagoonStaple_ask(w) {
    w.i({desc: 'the reader answers over a census it does not own — who calls the fixture method — with its enclosing def'})
...
LagoonStaple_lint(w) {
    w.i({desc: 'the lint answers over the same census — counting the corpus it was given and faulting on nothing'})
    let top = this.top_House()
    let lw = this.LagoonStaple_lw(w)
    if (!lw) return
...
LagoonStaple_seek(w) {
    w.i({desc: 'the seek is one answer over every standing census — and it names which ones replied'})
    let top = this.top_House()
    let lw = this.LagoonStaple_lw(w)
    if (!lw) return
...
LagoonStaple_beads(w) {
    w.i({desc: 'the shape of a document is held not computed — the defs inside its bead and the one beyond it'})
    let top = this.top_House()
    let lw = this.LagoonStaple_lw(w)
    if (!lw) return
...
LagoonStaple_refuse(w) {
    w.i({desc: 'a reader with no census refuses BY NAME — never a throw and never a silent empty'})
    let top = this.top_House()
    let SH = this.LagoonStaple_SH(w)
    let lw = this.LagoonStaple_lw(w)
    if (!SH || !lw) return
...
LagoonStaple_keeps_nothing(w) {
    w.i({desc: 'the reader kept nothing — after every answer its world holds only its own report row'})
    let lw = this.LagoonStaple_lw(w)
    if (!lw) return
