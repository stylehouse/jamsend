<script lang="ts">
    ...
    //    the player reaches Cello through BigSoundland, which imports Cellui DIRECTLY (no UI-registration
    ...
    // (BigSoundland mounts Cellui itself, gated to the player via critique_surface = !?B=.  It used to reach
    //  it through SchemeSwitcher, a renderer picker deleted 2026-09-09 — "we use Cello only now" — but the
    //   gate it carried is unchanged and is still the thing keeping Cello out of dispatch tabs.)
    })
</script>
