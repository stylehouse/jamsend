...
## ⚑ NIGHT OF 2026-09-11 — what landed unattended, and the one thing that reds every Lake Book
...
2. **FSA `expand()` stat'd files serially** — 139 files in `spec/` ≈ the 5s Wormhole budget, so the
    GhostList's 8s walk overran and retried forever. Now one `Promise.all` (`p2p/ftp/Directory.svelte.ts`).
     A/B'd against two red Lake Books: not the cause of their red (see 5).
3. **`Swarm_station_up`'s hello got the latch** (`Swarm_station_hello` + `Swarm_hello_retry` from the
    2s watch loop, capped 6, one deterministic seat). Verified on a headless music page: two acks, no
     retries. ⚠ Found while verifying: a FRESH identity's body name IS its soul name, so one ack answers
      both hellos — the first cut credited it to the body only and retried the soul to the cap. Fixed.
4. **The ×6200 dropped frames** — `pier_accept` + `ive_got` + `repli_lines` to two identities last seen
    in August, every beat, forever. Every sender checked the CARRIER; with a relay the carrier is always
     up. Gated `Swarm_deliver` and `Repli_offer` on `Presence_offline` (positive only; null sends;
      Books read null). Measured cadence before: ive_got every ~30s, pier_accept ~80s, repli_lines ~90s.
5. **⚠ EVERY LAKE BOOK IS RED, AND IT IS THE OWNER'S NEW INSTRUMENT.** Commit `1881f9f0` (2026-09-11)
    added `Lies_mark`: `performance.now()` milliseconds written as `sc` keys on a `see:lies` row in
     `w:Lies` — snapped into every Lake fixture, different every run. One diff line per Book:
      `see:lies,boot:63467,ghosts:…`. Moved the ms marks to `.c` (the console line still reads them);
       the row itself (`docs · wafts · ticks`) stays and is STILL new vs the fixtures — so LakeKeep
        LakeLocate LakeFunk LakeLango LakeFlush Interesting Engage Diffmatication Educarium Hackarium …
         stay red until re-sworn. **Owner's call: accept the row into the fixtures, or drop it from
          the snap.** (The `Editron` "step 001 mismatch" you saw today was this, not doc churn.)
6. **Waft:Everything + seven area Wafts** generated by `scripts/wafts_everything.mjs`; `≋` on a What
    with `FromWhat:Waft:…` jumps there (`ui/Waft.svelte`). CLAUDE.md points at it.
