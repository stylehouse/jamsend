...
**NEXT SHIFT — TEXT THAT TRULY KNOWS ITS SPACE (the human, 2026-07-18).**  The steer: *"I'm wanting
 to start another session about getting the text truly nice... it has to really know what the cell
  space is doing around each bit of it."*  Where that lands: the three seaters each know a DIFFERENT
   sliver — `pane_rows` its chord, `zone_seat` its band, `phi_build` its groove-gap — and none knows
    the space AROUND a placed bit (wall distance, neighbour pressure, the crater walls beneath, a
     sibling label's claim).  Candidates for the session (vague on purpose; the human steers):
 - a shared **space oracle** the seaters query per-bit: a free-room field over the cell (polygon +
    already-claimed boxes → "what room is around THIS word"), instead of each seater hand-rolling
     its own sliver of awareness;
 - placed text as first-class **claims** everywhere (the ϕ ledger generalised off the spiral): every
    bit records the region it holds, placement = query the field — never re-derive, never collide;
 - the **fit confidence** the human asked for: measure-after-place (`getBBox`/canvas `measureText`)
    closing the loop on the GLY estimate — today we PREDICT fit, we never CHECK it;
 - the polygon text-fit research forks (polylabel, textPath warp, wordcloud-mask packing) — PART 5.4
    in the history file has the google map.

**STATE OF THE GLASS (2026-07-18) — what stands, in one screen.**
 - **Three faces** ride `vsub_face` ($state): `tuples` (zone_seat territory rows) · `star` (the
    radial gem) · `phi` (the golden lettering layer).  Every tunable is a live KNOB, lovely
     configurations are named MOMENTS, and the whole op-rail grammar (`--face=vsub:phi`,
      `--face=knob:phi.pitch=20`, `--face=moment_save:name`, `--face=knobs`) lives in
       **`Attractor_todo.md`** — the dev-ground/competition housing: knobs are data, moments are
        recalls, faces ADD never replace.
 - **ϕ v2**: pole OFF-SCREEN (`phi.pole_dist`/`pole_ang` → gently curving near-parallel grooves),
    C-BLOCK claims ('Artist name: Vox' whole on one groove, children below + indented), STABLE
     pathids + `phi_prev` hysteresis (the post-drag blink fix — root cause was global-seq pathids
      recreating every DOM node), live streaming under drag (`phi.livedrag`), gap-determines-size
       text (fs fills the free gap up to `phi.fsceil`; titles may claim 2 grooves; wgt is claim
        PRIORITY now, not size), the title arc a `vsub_grab` drag handle.
 - **zone_seat** (the tuples-face text host): same-chord sizing+placement, split-don't-shrink,
    exact-or-folded (never a clipped glyph).  The growth spell runs under EVERY face (stat-blanking
     is render-time now, so the spell reads real sizes); crater why-not telemetry
      (`crater: {ok,small,header,geo,nofold}`) rides the vsub census — `--why` says exactly why a
       subcell didn't draw.
 - **Gang mirrors MERGE — the %gang_of blink loop fixed (2026-07-18).**  The mirror a gang-rep's
    Stuffing shows (`gang_stuff`, Cytui) was wiped + rebuilt on every membership change; on a
     streaming world (chunks landing, Stoker churn) that recreated the whole Stuffing DOM and pulsed
      its ⟳ commit spinner every beat — a permanent blink (seen live on BigSoundland).  Now the
       mirror merges DELTA-ONLY: a row whose sc still matches a member stays untouched (no version
        bump → no commit → no spinner), and the merge runs every wave so same-count swaps land too.
         NOTE the crusher still elects `gang[0]` as rep — membership flutter at the tiny-threshold
          can re-elect and re-mint the NODE itself; if blink persists at the CELL grain (not the
           rows), the fix is rep-election hysteresis in `Voro.g` (crusher-side, the human's call).
 - **Faces gated + ztuffing components + the unclipped mold (2026-07-19).**  Believing
    %face (worn AND imposed) is now COMMISSION-gated (`useFaces` → `w.c.use_faces` →
     `cyto_face_kind(w,n)`; Sounditron's commission already carried the flag); a faced
      fold|gang MEMBER mounts its component in a slot on its cell (`sub_faces_sync` — the
       ztuffing grain of the faces rail); molded overlays no longer clip (`stuff.clip` 0 +
        `.stuff-overlay` overflow:visible; ownership mark moved clipPath → `dataset.molded`).
         SAME-DAY walk-back on size ("they're too big now"): `stuff.scale` 2 → **1.5** (split
          the difference), and the final mold now lives in an ABSOLUTE band — `stuff.top` (2)
           / `stuff.bottom` (0.5) — so no chunk balloons past top× natural size however huge
            its cell ("one gets waaay too big") and none grinds below bottom×.  Plus the
...
**THE SHIFT LEDGER (PARTs 1–11, 2026-07-14 → 07-17; detail verbatim in
 ...
 1 honesty pass + C-space craters (B0/B1/B3) · 2 H-round: one coagulate + correspondence + ribbons ·
  3 I-round: hue=what-it-IS, inner shadow, badge dedup, growth spell, huddle · 4 J-round: kind pill
   + stretch · 5 J walk-back: de-button, name-drop fix, crater-always, polygon-fit research map ·
    6 C-cradle + drag-the-artist + spell hysteresis + hover-escape · 7 awake punch-list: de-box,
     badges gone, cradle reads, norot header, bloat-up, drag-freeze · 8 K-round: explicit nested
      basins + VoroScape deepened (fixture drift = the human's re-record) · 9 ϕ v1 + zone_seat +
       gates opened · 10 the Attractor Ground + zone_seat clip fix + spell-under-ϕ + crater
        telemetry · 11 ϕ v2 (off-screen pole, C-blocks, stable-identity blink fix, blobule drag,
         maxed text).

**THE FIVE WAVES (status only; full annals in the history file).**
 - **① the Se-up model — LANDED** (Voro_model in the crush tail; w:Voronoiology = the model's
    snapped face; mainkey = provenance).
 - **② VoroTest Book — LANDED headless** (15 `%see`, nine Examples); the LIVE gate + the human's
    re-record are OWED (see below).
 - **③ Cytui consumes the model — PART DONE** (one-walk gather, `Se:input`, Loud `share:N`, the
    openness split all landed).  REMAINS: `region_of`/`family_trait` as readers off
     `w.c.voro_model` (render-only, eyes-verified), and the namekey thread — which is ITS OWN CUT,
      see THE SEQUENCING TRAP under WHAT'S OWED.
 - **④ geometry folds into Voro.g** — later; will feel out the LangTiles parse-storm.
 - **⑤ the graphical phase — text rendering BEFORE washes: THIS IS THE CURRENT ARC** (ϕ +
    zone_seat above; the space-oracle session next; washes/C2/Wes-Wilson river lettering after).
...
**Three standing gotchas at the tab.** (1) RELOAD the runner tab first if its Vite HMR socket is
 dead ([[hmr-socket-dead-tell]]) — relay ops answer but `.svelte` edits silently don't land.
  (2) Any render round leaves the crush-Book fixtures stale; re-record is the human's.
   (3) **`runner_shot` CANNOT capture the HTML overlays** (the human's realisation, 2026-07-18):
    `shot` is `cy.png()` — the cytoscape CANVAS only — and `--svg` is the voronoi glass SVG only;
     the Svelte components mounted in the overlay layer (node Stuffings, the radio faces —
      TunerFace/StokerFace/HeistFace — anything in `overlay_container`) are DOM, invisible to both.
       A shot showing a bare cell where a radio face lives is the TOOL's blindness, not a render
        bug.  Overlay pixels = the human's eyes / an OS screenshot; a DOM-capture rail (e.g.
         SVG `<foreignObject>` snapshot) is unbuilt — decide only if blind-agent need arises.
...
- **The layout vortex intake (NEW 2026-07-19)** — *"any sort of matter tagged and dropped into
   the layout vortex"*; Radios throwing a bucketload of UI at the glass is the forcing case.
    Task 10 below (DESIGN-FIRST, seed only).
...
### 10. The layout vortex — any matter, tagged and dropped in — DESIGN-FIRST (seed 2026-07-19)

**The seed (the human, verbatim).**  *"Radios is starting to use Voro for throwing a bucketload
 of UI on the screen. and I really want some open+close handles in Voro... surely the guts should
  feel navigable."*  Then the mid-thought correction, which is the actual design: *"hmmm, no,
   whatever makes the model handles putting chunks of whatever C**, which become
    Stuffing|ztuffing|SvelteComponent-based... but there could be any sort of matter tagged and
     dropped into the layout vortex."*

Two readings to hold together (vague on purpose; the human steers):
- **The intake is the general thing.**  Radio already throws UI at the glass, but by bespoke
   wires: a face WORN (`sc.face`), a mainkey IMPOSED (`FACE_MAINKEYS`), ztuffing slots on a faced
    gang member (`sub_faces_sync`), CREWS/%Tuner show|hide — each its own path.  The vision is
     ONE tagging contract: any chunk of C** dropped into the vortex becomes layout matter, and
      the TAG picks its flesh — Stuffing (the default speech), ztuffing (the sub-face grain), or
       a full SvelteComponent face.  What Radio does ad hoc today becomes the front door for ANY
        matter.
- **Open+close falls out of the model, not out of bolted-on handles.**  The navigability want is
   real, but the walk-back says WHERE it lands: openness IS the model's dial already (crush level
    + pops — the lexicon), `Vtuff_pop`/`c.popped` IS the open gesture, and the model is allowed
     to know it (openness moves ON the remembered things, never decides who is remembered).  So
      no per-widget chrome — whatever handles the dropped matter carries its openness, and the
       open+close handles emerge wherever matter stands.

Design questions for the note (write it, get it agreed, THEN code):
- WHERE does the tag live?  A worn tag is the particle's own claim (`sc` — snapped, a Book can
   see it); an imposed tag is a viewer ruling (commission / c-side — Books stay Voro-blind,
    the imposed-from-above doctrine).  Probably BOTH, like faces today, gated the same way
     (`useFaces` → `w.c.use_faces`).
- WHAT is the contract between tag and flesh?  comp_ faces / ztuffing / Stuffing each want a
   different mount today; the model should hand the render ONE shape so Cytui stays WIRING +
    PAINT (the three-layer cleave, §0) — a dropped chunk's reading, not a renderer branch.
- HOW does dropped matter meet the crush?  Arbitrary chunks will gang|fold|stand-bare like
   anything else — is that wanted, or does tagged matter claim standing (the `c.popped`
    precedent for intent the crusher respects)?

x-ref: `Attractor_todo.md` (knobs/moments/faces — the op-rail this rides), `Radio_todo.md`
 (the radio faces as they stand), §0 "Faces gated + ztuffing components".
