# Radio_lowlevel.md — the ladder: instance-by-instance history of the Musu rebuild

The LOW-LEVEL half of what was `Radio_todo.md` (split out 2026-07-07, owner's call): the
 instance-by-instance pull of the old `Radios.svelte` machine into cursor simulations and
  tiny aspect proofs — **the ladder we climbed, not the direction we face.** Kept verbatim
   (original § numbers preserved — external references to `Radio_todo §3–§8` land here) for
    regression hunts and archaeology. The direction lives in `Radio_todo.md`: the spring
     framing, the `rastock → racast → raterm` pipeline (§3 there), and Pier reality (§9).
      Board twin: the Credence group `What:Musu / What:lowlevels`. Expect Books documented
       here to RETIRE as the `Ra*` re-draws go green — nothing is deleted until its re-draw
        stands, and nothing here should steer new work.
