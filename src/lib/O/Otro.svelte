<script lang="ts">
    ...
    // ── the todo spool — EXTRACTED to ui/TodoSpool.svelte (2026-08-30) so the same flood-tracer
    //  rides BigSoundland's guts sprawl headers.  `spool_open` (module $state, one-at-a-time
    //   across every instance on the page) is imported only for the has-open-todo z-index lift.
    ...
    {#snippet children({ scrollToHouseIdx, scrollToHouseIp, childrenOf })}
        {#each houses as house, i (house.c.ip)}
            ...
            <div class="house-header"
                ...
                class:has-open-todo={spool_open.ip === house.c.ip}
                id="house-{house.c.ip}"
...
<style>
    ...
    /* the drain-queue badge (TodoSpool) rides the end of the name flex — push it right, as
       the inline .todo-count used to sit.  All spool styling lives in ui/TodoSpool.svelte. */
    .house-name :global(.todo-count) { margin-left: auto; }
