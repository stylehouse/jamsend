...
// rendered_rows: $derived so the {#each} source is reactive to the frame counter.
//   anim_rows is plain (mutations don't signal) — the each would freeze at its
//   first-frame opacity if it read anim_rows directly.  Reading anim_version
//   here makes the derived re-run each tick, pulling current ar.y|ar.opacity.
let rendered_rows = $derived(anim_version >= 0 ? anim_rows : [])
...
function anim_tick() {
    ...
    for (let i = anim_rows.length - 1; i >= 0; i--) {
        ...
        if (Math.abs(dop) > 0.005) { ar.opacity += dop * 0.25; moving = true }
...
let list_h = $derived(
    rendered_rows.length
        ? Math.max(...rendered_rows.map(ar => ar.y + ROW_H)) + 8
        : 40
)
