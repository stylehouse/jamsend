...
//#region Lang_graft_points
    ...
    async Lang_graft_points_once(w: TheC, dock: TheC) {
        ...
        // src_path comes from the live %What via Waft_src_doc_path (LangHold — the
    ...
    // Doc-from-src resolution lives in LangHold as Waft_src_doc_path — the old
    //   "each world keeps its own copy" stance let the copies drift (none of
    //   them saw a Doc nested deeper than one level); LangHold is dual-mounted,
