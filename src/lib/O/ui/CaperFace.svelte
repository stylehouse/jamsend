<script lang="ts">
    // CaperFace — the Pirating flow in the glass, one face per %Caper.  IMPOSED by mainkey
    //  (FACE_MAINKEYS, glass_faces.ts — no %Caper ever wears sc.face; the world and every
    ...
    let face = $derived.by(() => {
        ...
        const mainval = typeof sc.Caper === 'string' && sc.Caper !== '1' ? sc.Caper : ''
        return {
...
<style>
    .hf {
        pointer-events: none;
        width: max-content;
        /* THIS CAP IS THE PRICE CEILING, not a safety rail (the owner 2026-08-09: "the Caper glass is
           still way too skinny").  The measure pass stamps the face's NATURAL box and the solver buys
           the cell that box asks for — so a max-width here is the widest the Caper cell can ever bid
           for, and 300px bid for a sliver.  Raised with the same reasoning that widened the Heist
           face; a caper carries a sentence, a source and a lead row, all of which want a line. */
        max-width: 440px;
        padding: 7px 11px;
