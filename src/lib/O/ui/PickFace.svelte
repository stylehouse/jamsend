<script lang="ts">
    // PickFace — ONE kept track in a NESTED %Keep cell (the human 2026-07-28 "nodulate down the folder
    ...
    //    %KeepBar controls cell.  Click un-keeps it (drops the pick → the chip escorts out of the scope).  A
    //     landed pick wears ✓; the track you're hearing (the keep's seed) wears ♪.  Props { n, H } — n is the
    //      LIVE %Pick particle; n.c.up is its %Keep (stamped by Heist_keep_default_pick / _toggle).
    let { n, H } = $props()
    const A = H as any

    // the pull lands the record off the beat (pick.sc.landed flips on a tick), which a bare H.version derive
    //  can miss between bumps — poll a slow clock beside it (the KeepFace/UptimeFace idiom).
    let tick = $state(0)
...
<style>
    /* the whole chip is the un-keep button (KeepFace's .kf-track precedent); pointer-events:auto re-arms
