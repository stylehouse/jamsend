<script lang="ts">
    // HeistFace — the ⇊ "keep what you're hearing" heist AS A TIDY VYTO CELL (the human 2026-07-28: "I DO
    ...
    //    started").  One face per %Heist (FACE_MAINKEYS/glass_kinds, imposed by mainkey — no %Heist wears
    //     sc.face).  NOT fullscreen: it sits in the clutter while PRIMED, then FOLDS DOWN to a compact
    //      progress strip once it auto-starts pulling.  Props { n, H } — n is the LIVE %Heist particle.
    //
    ...
    //    non-reactive, so `n?.sc?.Heist` subscribes to nothing — only the bare `n` prop read does).
    ...
    lifewatch(H, 'face:Heist', () => String(n?.sc?.Heist || n?.sc?.id || '?'))
    ...
    let face = $derived.by(() => {
        ...
        return {
            ...
            // the REAL dest-root (see HeistBarFace): no invented `music/` root, and no `Unfiled` folder —
...
<div class="kf" class:folded={face.folded || face.unfocused}>
    <!-- THE HEAD IS THE PARTICLE (the human 2026-08-07: "better than opening the x8 tracks thing is to
         ...
         learn.  The TRACK NAME moves down below the filing rows: you choose where it lands first, then
         confirm what it is.  (There is no disclosure to open any more — the track list went 2026-08-09.) -->
    <div class="kf-head">
...
<style>
    /* the voronoi cell bbox overlaps its neighbours, so the ROOT must not eat pointer events; each
       button re-arms pointer-events (the CaperFace hard contract). */
    .kf {
        pointer-events: none;
        width: max-content;
