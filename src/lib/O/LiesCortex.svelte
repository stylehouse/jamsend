<script lang="ts">
    // LiesCortex.svelte — the compile-and-settle FOREMAN for w:Lies.
    //
    // Wires: A:Lies / w:Lies  (Cortex, Codebit + the gen-path / codetype helpers)
    //
    //   The runner DOWNSTREAM of a settled compile — req:Rundown, req:BlatDo, and the
    //   Pantheate receiver/executor (w:Pantheate) — was carved out into LiesRun.svelte.
    //   req_Cortex still pumps req:Rundown across the split (one H.* eatfunc table).
    //   The particle-layout below still maps the whole w:Lies/w:Pantheate req tree;
    //   its Rundown / BlatDo / Pantheate entries now live in LiesRun.
    //
    // ── What a Cortex is ──────────────────────────────────────────────────────
    //
    `
