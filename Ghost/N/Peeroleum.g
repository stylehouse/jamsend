...
async Peeroleum(A,w):
    oai seemingly:`ya aooooolly`...figaro:Flibbersnatch
...
say_hello(w, pier):
    ...
    hello.i({said:1, seq})
    &Peeroleum_send,w,{header:{type:'hello', from:me, to:pier%pub, seq, publicKey:me}}
...
hear_hello(w, pier, frame):
    const H = this
    let h = frame.header
    if (!String(h.publicKey).startsWith(pier%pub)) return false
