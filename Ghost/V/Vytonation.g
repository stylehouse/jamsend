...
VytoMitosis_found(w):
    i %desc:'found a flora of three genera and commission the glass on them'
    let cogs = []
...
VytoMitosis_grow(w, genera, doses, wait, want):
    i %desc:'grow the flora — newcomers arrive and the cut re-seats'
    let gi = 0
...
VytoMitosis_die(w, genus):
    i %desc:'a genus goes extinct — its cell escorts out and the survivors re-seat'
...
VytoMitosis_close(w):
    i %desc:'stir the unchanged world — the settled flora grants no motion'
    w.c.closing = 1
    this.Vyto_rest_reset(w)
...
VytoMitosis_witness(w):
    let vw = this.VytoStaple_vw(w)
    if (vw && vw.c.commission && vw.o({ Organ: 1 }).length === 10 && vw.o({ Bar: 1 }).length === 7 && !(oa %see:'the glass was commissioned on a flora of three genera and stood its board')) i %see:'the glass was commissioned on a flora of three genera and stood its board'
    if (this.Vyto_cells_at(w, 3) && !(oa %see:'the flora took distinct seats — three genera cut into three cells inside the frame')) i %see:'the flora took distinct seats — three genera cut into three cells inside the frame'
    if (this.Vyto_cells_at(w, 4) && !(oa %see:'a lone genus arrived and joined near the crowd — the flora re-seated around a fourth cell')) i %see:'a lone genus arrived and joined near the crowd — the flora re-seated around a fourth cell'
    if (this.Vyto_cells_at(w, 6) && !(oa %see:'a batch of genera arrived and spread around the rim — the flora re-seated as six cells')) i %see:'a batch of genera arrived and spread around the rim — the flora re-seated as six cells'
    if (this.VytoMitosis_extinct(w) && !(oa %see:'a genus went extinct — its cell wore the departing escort then the ring shrank and the survivors re-seated')) i %see:'a genus went extinct — its cell wore the departing escort then the ring shrank and the survivors re-seated'
    if (w.c.closing && w.c.settled && !(oa %see:'an unchanged world granted no motion — the settled flora stood byte-identical')) i %see:'an unchanged world granted no motion — the settled flora stood byte-identical'
...
VytoRadio_seed(w):
    i %desc:'seed the six-genus tuner flora and commission the glass'
    let cogs = []
...
VytoRadio_drift(w):
    i %desc:'the doses drift — the tuner ages and the cells re-size and re-seat'
    let vw = this.VytoStaple_vw(w)
...
VytoRadio_hand(w):
    i %desc:'the hand pins one cell while the drift rearranges the rest'
    let vw = this.VytoStaple_vw(w)
...
VytoRadio_release(w):
    i %desc:'the hand lets go — the hold eases to free and retires itself'
    let vw = this.VytoStaple_vw(w)
...
VytoRadio_witness(w):
    let vw = this.VytoStaple_vw(w)
    if (vw && vw.c.commission && vw.o({ Organ: 1 }).length === 10 && vw.o({ Bar: 1 }).length === 7 && !(oa %see:'the tuner glass was commissioned on six genera and stood its board')) i %see:'the tuner glass was commissioned on six genera and stood its board'
    if (this.Vyto_cells_at(w, 6) && !(oa %see:'six genera took distinct seats sized by their dose — the tuner flora cut into the frame')) i %see:'six genera took distinct seats sized by their dose — the tuner flora cut into the frame'
    if (this.VytoRadio_drifted(w) && !(oa %see:'the doses drifted and the express organ re-sized the cells — the flora re-seated as it aged')) i %see:'the doses drifted and the express organ re-sized the cells — the flora re-seated as it aged'
    if (this.VytoRadio_held(w) && !(oa %see:'the hand held one cell in its seat while the drifting world rearranged around it')) i %see:'the hand held one cell in its seat while the drifting world rearranged around it'
    if (this.VytoRadio_freed(w) && !(oa %see:'the hand let go — the hold eased back to free and retired itself')) i %see:'the hand let go — the hold eased back to free and retired itself'
...
VytoWeb_seed(w):
    i %desc:'seed eight sibling cogs — four brass four iron and unique teeth'
...
VytoWeb_commission(w):
    i %desc:'mint A:Vyto beside the run and commission it on the eight cogs'
    this.Vyto_commission_on(w, w.c.cogs, 1)
...
VytoWeb_prime(w):
    i %desc:'stir to rest — gang elects its key and relate weaves the edges'
    this.Vyto_rest_reset(w)
...
VytoWeb_focus(w):
    i %desc:'propose focus on one cog through the governor door'
    let vw = this.VytoStaple_vw(w)
    let cells = this.Vyto_cells(vw)
...
VytoWeb_release(w):
    i %desc:'release the focus — the governor stands down and the cut evens'
    let SH = this.VytoStaple_SH(w)
...
VytoWeb_witness(w):
    if (this.VytoWeb_board_ready(w)) {
        ...
        if (!(oa %see:'the glass was commissioned on eight sibling cogs and stood its board')) i %see:'the glass was commissioned on eight sibling cogs and stood its board'
    }
    if (this.VytoWeb_gang_ready(w)) {
        ...
        if (!(oa %see:'the gang organ elected metal — the one key that cleaves eight cogs into brass and iron')) i %see:'the gang organ elected metal — the one key that cleaves eight cogs into brass and iron'
    }
    if (this.VytoWeb_relate_ready(w)) {
        ...
        if (!(oa %see:'the relate organ wove twelve flow edges — every pair sharing a metal and none that do not')) i %see:'the relate organ wove twelve flow edges — every pair sharing a metal and none that do not'
    }
    if (this.VytoWeb_dom_ready(w)) {
        ...
        if (!(oa %see:'the focus governor swelled the chosen cog to dominance — attention became geometry')) i %see:'the focus governor swelled the chosen cog to dominance — attention became geometry'
    }
    if (this.VytoWeb_even_ready(w)) {
        this.story_swear(w, 'released focus let the cut breathe even again — no decision no taper')
        if (!(oa %see:'released focus let the cut breathe even again — no decision no taper')) i %see:'released focus let the cut breathe even again — no decision no taper'
    }
...
VytoFold_budget(w):
    i %desc:'the screen budget scales with the glass area'
...
VytoFold_open(w):
    i %desc:'a roomy glass folds nothing'
...
VytoFold_least(w):
    i %desc:'the least-dominant family folds first'
    let t = this.VytoFold_tree(w)
...
VytoFold_shield(w):
    i %desc:'the path to the focus is shielded from folding'
    let t = this.VytoFold_tree(w)
    let leaf = t.A.kids[0]
...
VytoFold_cohere(w):
    i %desc:'a sub-floor family distils to buckets while a bucketless one is crushed'
    let t = this.VytoFold_tree(w)
...
VytoFold_gang_same(w):
    i %desc:'a crowded row of identical members elects no representative'
    let members = []
    let idx = 0
    while (idx < 7) {
...
VytoFold_gang_uniq(w):
    i %desc:'a crowded row of all-distinct members elects no representative'
    let members = []
    let idx = 0
    while (idx < 7) {
...
VytoFold_witness(w):
    if (w.c.fold_budget) {
        ...
        if (!(oa %see:'the screen budget grows with the glass — a wall four times the area introduces four times as many cells')) i %see:'the screen budget grows with the glass — a wall four times the area introduces four times as many cells'
    }
    if (w.c.fold_open) {
        this.story_swear(w, 'a roomy glass folds nothing — every family stands open when the budget holds every cell')
        if (!(oa %see:'a roomy glass folds nothing — every family stands open when the budget holds every cell')) i %see:'a roomy glass folds nothing — every family stands open when the budget holds every cell'
    }
    if (w.c.fold_least) {
        ...
        if (!(oa %see:'the glass folds its least-dominant family first — the smallest-membered row collapses while larger siblings stand open')) i %see:'the glass folds its least-dominant family first — the smallest-membered row collapses while larger siblings stand open'
    }
    if (w.c.fold_shield) {
        ...
        if (!(oa %see:'the path to the focus is never folded — attention holds every cell from the root down to the held leaf open however tight the glass')) i %see:'the path to the focus is never folded — attention holds every cell from the root down to the held leaf open however tight the glass'
    }
    if (w.c.fold_cohere) {
        ...
        if (!(oa %see:'a family whose members fall below the coherence floor distils to buckets while a bucketless family is crushed whole')) i %see:'a family whose members fall below the coherence floor distils to buckets while a bucketless family is crushed whole'
    }
    if (w.c.fold_gang_same) {
        ...
        if (!(oa %see:'a crowded row of identical members elects no representative — a uniform family has none to stand for it')) i %see:'a crowded row of identical members elects no representative — a uniform family has none to stand for it'
    }
    if (w.c.fold_gang_uniq) {
        ...
        if (!(oa %see:'a crowded row of all-distinct members elects no representative — an all-unique family has no key that partitions it')) i %see:'a crowded row of all-distinct members elects no representative — an all-unique family has no key that partitions it'
    }
...
VytoBunch_seed(w):
    i %desc:'seed four cogs — two oak kin split by the strangers pine and birch'
    let cogs = []
...
VytoBunch_commission(w):
    i %desc:'mint A:Vyto beside the run and commission it on the four cogs'
    this.Vyto_commission_on(w, w.c.cogs, 1)
...
VytoBunch_rest(w):
    i %desc:'stir to rest — relate weaves the one kin edge and the attraction bunches the oaks'
    this.Vyto_rest_reset(w)
...
VytoBunch_sever(w):
    i %desc:'sever the kinship — retag one oak so the edge drops and the pair rests free'
    let vw = this.VytoStaple_vw(w)
...
VytoBunch_witness(w):
    if (this.VytoBunch_board_ready(w)) {
        ...
        if (!(oa %see:'the glass was commissioned on four cogs and stood its board')) i %see:'the glass was commissioned on four cogs and stood its board'
    }
    if (this.VytoBunch_edge_ready(w)) {
        ...
        if (!(oa %see:'the relate organ wove one kin edge — the two oaks share a genus and the strangers share nothing')) i %see:'the relate organ wove one kin edge — the two oaks share a genus and the strangers share nothing'
    }
    if (this.VytoBunch_distinct_ready(w)) {
        ...
        if (!(oa %see:'the solve seated four distinct cells — the attraction drew the kin close but never collapsed them')) i %see:'the solve seated four distinct cells — the attraction drew the kin close but never collapsed them'
    }
    if (this.VytoBunch_bunched_ready(w)) {
        ...
        if (!(oa %see:'the shared genus bunched the kin — the two oaks rest closer to each other while joined than once their kinship is severed')) i %see:'the shared genus bunched the kin — the two oaks rest closer to each other while joined than once their kinship is severed'
    }
...
VytoBreathe_seed(w):
    i %desc:'seed four cogs — three oak kin sharing a genus and a lone pine all at the same dose'
    let cogs = []
    cogs.push(w.i({ Cog: 'oak1', of: 'main', genus: 'oak' }))
    cogs.push(w.i({ Cog: 'oak2', of: 'main', genus: 'oak' }))
...
VytoBreathe_price(w):
    i %desc:'commission the glass priced on the four cogs and stir to rest'
...
VytoBreathe_unprice(w):
    i %desc:'re-commission the same kin cogs unpriced — shared meaning now buys no size'
...
VytoBreathe_sever(w):
    i %desc:'sever the kinship then re-commission priced — with no shared genus the oaks shrink to the pine'
    let cogs = w.c.cogs
...
VytoBreathe_witness(w):
    if (this.VytoBreathe_board_ready(w)) {
        ...
        if (!(oa %see:'the glass was commissioned priced on four cogs and stood its board')) i %see:'the glass was commissioned priced on four cogs and stood its board'
    }
    if (this.VytoBreathe_kin_bigger_ready(w)) {
        ...
        if (!(oa %see:'a cell tied by shared meaning is a bigger cell — each oak sharing its genus outsizes the unrelated pine though every cog carries the same dose')) i %see:'a cell tied by shared meaning is a bigger cell — each oak sharing its genus outsizes the unrelated pine though every cog carries the same dose'
    }
    if (this.VytoBreathe_kin_even_ready(w)) {
        ...
        if (!(oa %see:'the three kin cells are sized evenly — each oak carries the same shared-meaning lift so the three take equal seats larger than the lone pine')) i %see:'the three kin cells are sized evenly — each oak carries the same shared-meaning lift so the three take equal seats larger than the lone pine'
    }
    if (this.VytoBreathe_unpriced_equal_ready(w)) {
        ...
        if (!(oa %see:'without the scale shared meaning buys no size — the same kin cogs take equal base seats when the glass is priced plainly')) i %see:'without the scale shared meaning buys no size — the same kin cogs take equal base seats when the glass is priced plainly'
    }
    if (this.VytoBreathe_severed_equal_ready(w)) {
        ...
        if (!(oa %see:'sever the kinship and the size follows — with no shared genus the priced oaks shrink back to the lone pine seat')) i %see:'sever the kinship and the size follows — with no shared genus the priced oaks shrink back to the lone pine seat'
    }
...
VytoNest_seed(w):
    i %desc:'seed a two-level rig — three cogs under the root and two grandchildren under the first'
    let rig = w.i({ Rig: 'main' })
...
VytoNest_nest(w):
    i %desc:'commission the glass nested on the rig and stir to rest'
...
VytoNest_flat(w):
    i %desc:'re-commission the same tree flat — only the root is cut and the children get no target'
...
VytoNest_witness(w):
    if (this.VytoNest_board_ready(w)) {
        ...
        if (!(oa %see:'the glass was commissioned nested on one rig and stood its board')) i %see:'the glass was commissioned nested on one rig and stood its board'
    }
    if (this.VytoNest_root_tiles_ready(w)) {
        ...
        if (!(oa %see:'the scope recursion fills the frame — the three cogs tessellate the whole root cell so the parent area equals the sum of its children')) i %see:'the scope recursion fills the frame — the three cogs tessellate the whole root cell so the parent area equals the sum of its children'
    }
    if (this.VytoNest_deep_tiles_ready(w)) {
        ...
        if (!(oa %see:'the recursion goes deeper — the two grandchildren tessellate their parent cog cell exactly so a scope nests inside a scope')) i %see:'the recursion goes deeper — the two grandchildren tessellate their parent cog cell exactly so a scope nests inside a scope'
    }
    if (this.VytoNest_all_seated_ready(w)) {
        ...
        if (!(oa %see:'every nested cell took a seat inside the frame — each child and grandchild wears a target with a real radius')) i %see:'every nested cell took a seat inside the frame — each child and grandchild wears a target with a real radius'
    }
...
VytoOrchestra_spotlight(w):
    i %desc:'focus one song — its cell must swell while an off-focus sibling compresses'
    let vw = this.VytoStaple_vw(w)
    let pre = {}
    if (vw) {
        for (const r of this.Vyto_cells(vw)) {
...
VytoOrchestra_witness(w):
    ...
    if (w.c.saw_rim) {
        ...
        if (!(oa %see:'the strays ride the rim — a loose row takes no seat in the pile and feels no wall')) i %see:'the strays ride the rim — a loose row takes no seat in the pile and feels no wall'
    }
    if (w.c.saw_weave) {
...
VytoKindfold(A,w):
    w oai %req:wrangle,eternal
        await &VytoKindfold_drive,w,req
        req%ok = 1
...
VytoKindfold_seed(w):
    i %desc:'seed eighteen members across three kinds — eight cogs six vanes four hubs all sharing a metal'
    let members = []
...
VytoKindfold_crowd(w):
    i %desc:'commission the glass folded with no deck — the wall falls on the discovered key'
...
VytoKindfold_kind(w):
    i %desc:'re-commission the same eighteen with the kind ladder engaged — the wall falls on kind'
...
VytoGrasp_seed(w):
    i %desc:'seed a field of five cogs — all sharing one join one wearing a claim nobody else makes'
    let cogs = []
    let k = 0
    while (k < 5) {
        let sc = {}
...
VytoGrasp_stand(w):
    i %desc:'commission the glass and stand the grasp — the field reads its loud and quiet claims'
    this.Vyto_commission_on(w, w.c.cogs, 1)
...
VytoGrasp_change(w):
    i %desc:'two cogs arrive and one leaves — the grasp names the arrivals and the departure'
...
VytoTwin_seed(w):
    i %desc:'seed two byte-identical siblings beside one that differs'
...
VytoTwin_stand(w):
    i %desc:'commission the glass — the twins share one cell and that cell says it stands for two'
    this.Vyto_commission_on(w, w.c.cogs, 1)
...
VytoTwin_flat(w):
    i %desc:'give a cog three hidden children and mark it flat — the cell must say how many it hides'
