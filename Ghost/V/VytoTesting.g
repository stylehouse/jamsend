...
VytoPosed_seed(w):
    i %desc:'seed five Tree faces at uneven dose'
...
VytoPosed_slab(w):
    i %desc:'commission the glass polygon-cut — five real faces, walls to lean into'
...
VytoPosed_ball(w):
    i %desc:'re-commission the same faces foam-cut — a ball poses level'
...
VytoPosed_witness(w):
    if (w.c.saw_slab) {
        ...
        if (!(oa %see:'a genuinely slanted wall leans its face to match — the pose never strays past thirty degrees')) i %see:'a genuinely slanted wall leans its face to match — the pose never strays past thirty degrees'
    }
    if (w.c.saw_ball) {
