...
VoroMitosis_witness(w):
    ...
    if (cells.length && folded.length === cells.length && !(oa %see:'every living genus is crush-folded behind one chunk keyed to its own name')) i %see:'every living genus is crush-folded behind one chunk keyed to its own name'
//#endregion
