...
MusuRaStock_witness(w):
    ...
    if (n === 3 && shape_ok && !(oa %see:'every Record stands preview first — a cached window and a stream head that begins right after it')) i %see:'every Record stands preview first — a cached window and a stream head that begins right after it'
...
MusuRaCast_witness(w):
    ...
    if (n === 8 && whole && !(oa %see:'the listener pulled the whole Record — the cached preview byte-faithful and the continuation transcoded across the wire')) i %see:'the listener pulled the whole Record — the cached preview byte-faithful and the continuation transcoded across the wire'
...
MusuRaStream_witness(w):
    ...
    if (n >= 6 && s && s.a_drops > 0 && s.a_heard > s.b_heard + 2 && !(oa %see:'the slow transcode clock fell behind the playhead — the tail of the stream starved with real holes')) i %see:'the slow transcode clock fell behind the playhead — the tail of the stream starved with real holes'
    ...
    if (n >= 6 && s && s.b_heard <= 3 && !(oa %see:'changing the track resumed a fed stream — a fresh preview primed and the transcoder kept ahead at the healthy rate')) i %see:'changing the track resumed a fed stream — a fresh preview primed and the transcoder kept ahead at the healthy rate'
