...
//   predicate, and Atlas stamps `%Doc,testing` from it).  The Book NAME did not move with the file —
//    `MusuStaple` and every fixture under `wormhole/Story/` are untouched by a rename.
// the Musu* music-piracy tests, in the Pere* mould (spec: Radio_todo.md).  The file
...
// ── the REPLICATION PROTOCOL likewise moved out to Ghost/N/Repli.g (same regroup): the paginated streaming
//  C** replication (this.Repli_*) is shared real software; the MusuReplica / MusuReco Books below still
//   compose it from the outside, callers unchanged (the verbs already carried the Repli_ family prefix, so
//    the ~49 call sites needed no rename).  The Selection import went with it — nothing here uses it now.
...
// ── the REPLICATION PROTOCOL grew up here (//#region repli: Repli_loc_keys … Repli_sent_se) and MOVED to
//  Ghost/N/Repli.g (Radiobuddies regroup — spec/Radiobuddies_handover.md): shared real software above the
//   transport spine, called cross-ghost (this.Repli_*).  MusuReplica / MusuReco below are its users.
...
MusuReco_witness(w):
    ...
    if (parked_signals >= 2 && !(oa %witnessed:parked_signalled)) i %witnessed:parked_signalled
    ...
    if (parked_signals >= 2 && !(oa %see:'the sink was told by wire when its want got parked — not left to infer it from silence')) i %see:'the sink was told by wire when its want got parked — not left to infer it from silence'
//#endregion
