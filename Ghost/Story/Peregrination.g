...
async Lake_order(w):
    ...
    let peer = (a) => ['Alice', 'Bob', 'Ivy', 'Jon', 'Kim', 'Lee'].includes(a%A) ? 0 : 1
...
Lake_sides_up(w):
    ...
    H i A:Alice$:AliceA/w:Peeroleum$:Alicew
    ...
    &Peeroleum_send,Alicew,{header:{type:'noop', from:'alice', to:'bob', seq:s}}
...
async Lake_trial_arm(w):
    ...
    &Socket,Alicew
    &Socket,Bobw
    &Tribunal_pair_websocket,Alicew,Bobw
    &Tribunal_hand_to_webrtc,Alicew
    &Tribunal_hand_to_webrtc,Bobw
...
async Lake_trial_fallback(w):
    ...
    &Tribunal_fall_to_websocket,Alicew
    &Tribunal_fall_to_websocket,Bobw
    let s = this.Pier_next_seq(AlicePier)
...
async Lake_stall_arm(w):
    w i %reached:step_11
    H i A:Kim$:KimA/w:Peeroleum$:Kimw
    H i A:Lee$:LeeA/w:Peeroleum$:Leew
    Kimw i Peering,name:kim$:KimPeering
    Leew i Peering,name:lee$:LeePeering
    KimPeering.c.up = Kimw
    LeePeering.c.up = Leew
    let KimPier = KimPeering oai Pier,pub:lee,req
    let LeePier = LeePeering oai Pier,pub:kim,req
    &transport,KimA,Kimw
    &transport,LeeA,Leew
    &Peeroleum_arm_whittle,Kimw
    &Peeroleum_arm_whittle,Leew
    Kimw o active_transport$:Kimport.c.connection
    Leew o active_transport$:Leeport.c.connection
    ...
    &Peeroleum_send,Kimw,{header:{type:'noop', from:'kim', to:'lee', seq:s}}
...
Lake_witness(w):
    ...
    H o A:Kim/w:Peeroleum/Peering/Pier$:KimPier2
