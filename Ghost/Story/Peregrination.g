...
async Lake_proof_drive(w, req):
    ...
    if n != null && n !== req.c.did_step
        ...
        if n === 2
            ...
            await &Lake_corrupt_redial_arm,w
        else if n === 32
            await &Lake_silence_retx_arm,w
        else if n === 33
            await &Lake_crossfire_arm,w
    await &Lake_pump_handshakes,w
...
Lake_proof_see(w):
    ...
    if (crfaulterr === 'bad-body-hash' && crheld >= 2 && crcleared && crrecovered && crgood && !(oa %see:'corrupt mid-re-dial frame faulted not lost — good tail recovered')) i %see:'corrupt mid-re-dial frame faulted not lost — good tail recovered'
    ...
    if (srheard && srmid && srstalled && srsilent && !(oa %see:'silence latched mid-retransmit — then the stall joined it — both carrier-down signals coexist')) i %see:'silence latched mid-retransmit — then the stall joined it — both carrier-down signals coexist'
    ...
    if (cfclean && cfhealed && cfcorrupt && !(oa %see:'three interleaved streams routed by identity stayed isolated — clean delivered — lossy healed — corrupt faulted alone')) i %see:'three interleaved streams routed by identity stayed isolated — clean delivered — lossy healed — corrupt faulted alone'
...
Run_A_PereReborn():
    this.c.role ??= 'runner'
    H i A:PereReborn/w:PereReborn

PereReborn(A,w):
    w oai %req:wrangle,eternal
        await &PereReborn_drive,w,req
        req%ok = 1
...
async PereReborn_drive(w, req):
    ...
    if n != null && n !== req.c.did_step
        req.c.did_step = n
        if n === 2
            await &PereReborn_link,w
        else if n === 3
            await &PereReborn_flow,w
        else if n === 4
            await &PereReborn_reborn,w
        else if n === 5
            await &PereReborn_collision,w
        else if n === 7
            await &PereReborn_epoch,w
    await &Lake_pump_handshakes,w
...
PereComplain(A,w):
    w oai %req:wrangle,eternal
        await &PereComplain_drive,w,req
        req%ok = 1
