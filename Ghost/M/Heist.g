...
async Heist_keep_beat(w, ident):
    ...
    // THE HEARD MAG, LIVED OUT (Radio_circuit_todo §4).  Here rather than earlier because it MINTS into
    //  the same shop the loop below walks, and a keep minted after the loop would idle a whole beat before
    //   its first step.  Bounded to one mint per pass and one live keep per holder inside; nav-gated, so a
    //    Book (which has no Crate_nav) never spontaneously grows a heist out of a seeded take.
...
// Heist_keep_take_go — A ♥ KEEP HAS NO FORM AND NO HUMAN EITHER (the owner 2026-09-03: *"turn the heist
...
async Heist_keep_cancel(w, keep):
    ...
    this.Heist_blag_drop(keep.sc.seed)
    // A ♥ KEEP'S ✕ RETIRES THE HEART TOO, or the haul beat mints it straight back next pass and the
    //  button reads as broken by doing exactly what it was told.  This is the ONE road by which a take
    //   ever ends (Heard_untake, §C: "you can't lose a heart — only you retire one"): the machine never
    //    calls it, the clock never calls it, a human pressing ✕ calls it.
    if (keep.sc.take && keep.sc.pub && keep.sc.seed) {
        let rw0 = (this.top_House ? this.top_House().c.radio_w : null) || w
        try { this.Heard_untake(rw0, this.Radio_pub(rw0) || '', keep.sc.pub, keep.sc.seed) } catch (er) {}
    }
...
// ── WHO ASKS FOR WHAT, AND WHY IT IS NOT IN THIS FILE ANY MORE ─────────────────────────────────────
//  (2026-09-04, Radio_circuit_todo.md.)  The batch-acquisition block that stood here — Heist_wants_cap,
//   Heist_want_shelf/liked/open/jams/settle/off/keep/beat, Heist_haul_piers/carrying, and the arrivals
//    mirror Heist_newly_mirror/ids — read the %Jam ledger (%Jam,with:<dj> > %Like,of / %Grab,of) that
//     Jam.g minted.  The owner's verdict on that shape was *"cursed … a big cancer"* and it is gone
//      whole: the ledger is now ONE Mag under my own identity, `%Mag:heard,pub:<me>`, holding one
//       `%Card,id,pub` per track I ever heard, and a heist in progress is a QUERY over it rather than
//        any particle at all.  See **Ghost/M/Heard.g**, which owns every verb that moved:
//          Heist_want_liked  → Heard_taken          Heist_want_beat   → Heard_haul_beat
//          Heist_want_open   → Heard_takes          Heist_want_keep   → Heard_keep
//          Heist_want_jams   → Heard_takes          Heist_want_off    → Heard_untake
//          Heist_want_settle → Heard_landed (derived — there is no got-mark to write any more)
//          Heist_haul_piers  → Heard_haul_piers     Heist_newly_ids   → Heard_landed_ids
//  WHAT STAYED HERE, and why: the whole BYTE LANE.  A `%Heist` keep under `Ra_home_shop` is still what
//   carries one track's bytes — Rummage, the bay, Repli, materialise, quarantine, vouch, land — and the
//    heard circuit mints one, lets it finish, and reads it back.  The keep is scaffolding now (transient,
//     one per holder, alive only while a Card is being carried); the Card is the intent and the ledger.
//      `Heist_keep_take_go` (ex `_like_go`) is where the two meet.
