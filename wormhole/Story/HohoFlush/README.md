...
# HohoFlush — what each step should do

The story forks HohoNets but swaps the grab-bag of marks for a tight push|pull
...
`Doc:Ghost/test/Story/Hoho/HohoAntecedents.g` and `Point,method:Peeroleum`.  The flush (encode /
...
Loads `Waft:Ghost/HohoFlush` from the snap.  Nothing armed yet; LE.sc.target absent.
...
- working clones = [`Doc:Ghost/test/Story/Hoho/HohoAntecedents.g`, `Point,method:Peeroleum`]
...
- replace_back writes working's clones back as `What:peer` children, so the OC
    `Waft:Ghost/HohoFlush/What:foundations/What:peer` now also holds
...
- Lang: target is in Ghost/HohoFlush -> LE.c.origin_dirty = 1 -> ponder
